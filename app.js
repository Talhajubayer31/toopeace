...
const checkoutDivision = $('#checkout-division');
const checkoutDistrict = $('#checkout-district');

const bdDistricts = {
  Dhaka: ['Dhaka', 'Faridpur', 'Gazipur', 'Gopalganj', 'Kishoreganj', 'Madaripur', 'Manikganj', 'Munshiganj', 'Narayanganj', 'Narsingdi', 'Rajbari', 'Shariatpur', 'Tangail'],
  Chattogram: ['Bandarban', 'Brahmanbaria', 'Chandpur', 'Chattogram', 'Cox\'s Bazar', 'Feni', 'Khagrachhari', 'Lakshmipur', 'Noakhali', 'Rangamati', 'Cumilla'],
...
// Dynamic District Dropdown Logic for Bangladesh
checkoutDivision?.addEventListener('change', () => {
  const division = checkoutDivision.value;
  if (!checkoutDistrict) return;

  // Clear existing options
  checkoutDistrict.innerHTML = '<option value="" disabled selected>Select District</option>';

  if (division && bdDistricts[division]) {
    checkoutDistrict.disabled = false;
    bdDistricts[division].forEach(district => {
      const opt = document.createElement('option');
      opt.value = district;
      opt.textContent = district;
      checkoutDistrict.appendChild(opt);
    });
  } else {
    checkoutDistrict.disabled = true;
    checkoutDistrict.innerHTML = '<option value="" disabled selected>Select Division first</option>';
  }
});
...
checkoutForm?.addEventListener('submit', (e) => {
  ...
  setTimeout(() => {
    ...
    // Manually reset District dropdown state after form reset
    if (checkoutDistrict) {
      checkoutDistrict.disabled = true;
      checkoutDistrict.innerHTML = '<option value="" disabled selected>Select Division first</option>';
    }
